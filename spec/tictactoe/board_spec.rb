...
describe TicTacToe::Board do  
  ...
  it "clears all the markings for the board" do
    ...
    @board.reset
    @board.squares.should_not include @player_value
  end
  
  context "marking the board" do
    it "marks the board" do
      @board.mark(0, @player_value)
      @board.available_squares.should_not include 0
    end
    
    context "raises an exception" do  
      it "when move is less than the first square" do
        lambda {@board.mark(-1, @player_value)}.should raise_error TicTacToe::SquareNotAvailableError
      end
    
      it "when move is higher than the last square" do
        lambda {@board.mark(9, @player_value)}.should raise_error TicTacToe::SquareNotAvailableError
      end
      
      it "when move is already mark" do
        @board.mark(0, @player_value)
        lambda {@board.mark(0, @player_value)}.should raise_error TicTacToe::SquareNotAvailableError
      end
  ...
  it "clears the value" do
    @board.mark(0, @player_value)
    @board.clear(0)
    @board.available_squares.should include 0
  end
  ...
  describe "rows" do
    ...
    it "the first element in the rows is the first row" do
      @board.mark(0, @player_value)
      rows = @board.rows
      rows[0].should == [@player_value, nil, nil]
    end
  
    it "returns the rows with the middle element as the middle rows" do
      @board.mark(3, @player_value)
      rows = @board.rows
      rows[@size/2].should == [@player_value, nil, nil]
    end
    
    it "returns the rows with the last elements as the last rows" do
      @board.mark(6, @player_value)
      rows = @board.rows
      rows[@size - 1].should == [@player_value, nil, nil]
    end
  ...
  describe "diagonals" do
    ...
    it "returns the diagonal squares with the marked value" do
      ...
      squares = [[nil, @player_value, nil], [nil, @player_value, nil]]
      @board.diagonals.should == squares
    end
  end
  ...
  describe "available squares" do
    ...
    it "doesn't return the marked square" do
      ...
      @board.available_squares.should_not include move
    end
